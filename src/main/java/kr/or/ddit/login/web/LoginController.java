...
public class LoginController extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		 ...
		 * logger.debug("cookie.getName : {} / cookie.getValue() : {}",cookie.getName(),
		 * cookie.getValue());
		 * 
		 ...
		 * }
		 * 
		 * // 요청에 담긴 파라미터 이름을 확인 logger.debug("요청에 담긴 파라미터 이름 확인");
		 ...
		 * 
		 * 
		 * // 요청에 담긴 파라미터를 관리하는 맵객체 확인 logger.debug("요청에 담긴 파라미터를 관리하는 맴객체 확인");
		 * logger.debug("req.getParameterMap() :"); Map<String, String[]> map =
		 * req.getParameterMap(); Set<String> keys = map.keySet(); Iterator<String> it =
		 * keys.iterator();
		 * 
		...
		// userid가 brown일 때 비밀번호가 brwonpass 인경우에 한해 로그인이 성공되었다고 판단
		// 그 외 경우는 로그인 실패로 간주
		// 로그인 성공시 : main.jsp로 forward
		// forward : 요청을 처리할 다른 jsp, servlet에게 위임
		//			서버 안에서 이루어 지는 작업으로 클라이언트 입장에서는 누가 응답을 생성 했는지 알 수 없음
