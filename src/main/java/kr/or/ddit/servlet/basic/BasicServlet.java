...
public class BasicServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// if(설정로그레벨 <= debug) {
			logger.debug("basicServlet.doget() userid paramter : " + req.getParameter("uesrid"));
		// }
			
		logger.debug("basicServlet.doget() userid paramter : {} {}", 
				req.getParameter("uesrid"), req.getParameter("password"));
		...
		// 로딩시 최초1회	사용자가 요청을 할때마다		서버 종료 or reload
		
		// init 메소드는 해당 서블릿에서 사용한 자원을 초기화 할 때 사용
		// 로딩시 최초1회 호출 : 로딩 되는 시점 ==> 해당서블릿으로 최초 요청이 들어왔을 때
		// 단 web.xml의 servlet 엘레먼트의 하위 엘레먼트인 load-on-startup 엘레먼트 값으로
		...
		//							*.jsp, *.jspx url-pattern에 따라 
		//							jsp라는 이름의 서블릿에서 처리(JspServlet)
